...
export default function FetchCSVData() {
    ...
    const fetchCSVData = () => {
        ...
        axios.get(csvUrl)    // Use Axios to fetch the CSV data
            .then((response) => {
                const parsedCsvData: any = parseCSV(response.data);        // Parse the CSV data into an array of objects
    ...
    function parseCSV(csvText: any) {
        ...
        const headers: any = rows[0].split(',');        // Extract headers (assumes the first row is the header row)
